        ...
        spawn_local(async move {
            ...
            match outcome {
                UploadOutcome::Completed => {
                    ...
                    wasmProgress::progress_manager().set_status(
                        &entry.file_key,
...
pub fn wasm_pause_all() {
    ...
    for file_key in &keys {
        let (s_agg, t_agg) = {
            ...
            (
                sess.get_aggregate_progress(None),
                sess.get_aggregate_progress(Some(&tid)),
            )
        };
...
async fn start_file_internal(
    ...
    match upload::start_decision(&file_hash) {
        StartDecision::Completed { file_key } | StartDecision::Resume { file_key } => {
            return Ok(file_key);
        }
...
async fn run_upload(file_key: &str, transfer_id: &str, file: web_sys::File) -> UploadOutcome {
    let file_size = file.size() as u64;
...
async fn upload_chunk_task(
    ...
    wasmProgress::progress_manager().complete_chunk(
        &file_key,
        part_number,
        chunk_size,
        s_agg,
    ...
    retry::run_with_retry_string_async(
        ...
        |_attempt| {
            ...
            async move {
                ...
                wasmProgress::progress_manager().update_in_flight(
                    &fk,
                    part_number,
                    0,
                    s_agg,
